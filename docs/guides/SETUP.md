...
**ถ้าติดตั้งด้วย Download ZIP:** ดาวน์โหลด ZIP เวอร์ชันใหม่ → แตกไฟล์เป็นโฟลเดอร์ใหม่ →
copy **`.env`** กับโฟลเดอร์ **`user/`** จากโฟลเดอร์เดิมมาใส่ → ในโฟลเดอร์ใหม่รัน
`docker compose pull && docker compose up -d` → โฟลเดอร์เดิมลบทิ้งได้
(ข้อมูล tip อยู่ใน docker volume ไม่ได้อยู่ในโฟลเดอร์ — ไม่หาย)

ของใน `user/` (settings, theme, เสียง) อยู่ครบเหมือนเดิมทุกครั้ง ดูว่าแต่ละเวอร์ชัน
เปลี่ยนอะไรได้ใน `CHANGELOG.md`
...
สามชั้น: (1) เขาต้อง**จ่ายเงินจริง**ถึงส่งข้อความได้ — ป่วนมีต้นทุนทุกครั้ง (2) เพิ่มคำต้องห้ามใน `user/settings.json` → `banned_words` ระบบกรองให้ (3) ตั้ง `show_message_min` ให้ข้อความโชว์เฉพาะยอดที่สูงพอ และถ้าโดนจริง refund ผ่าน dashboard ของ gateway ได้ (ดูเงื่อนไขการคืนเงินในหมายเหตุด้านล่าง) — เขาเสียเงินฟรี
...
ของพื้นฐาน (คำต้องห้าม, ยอดขั้นต่ำโชว์ข้อความ) แก้ `user/settings.json` ไม่ใช่โค้ด เสียง = วางไฟล์ที่ `user/web/sounds/alert.wav` สี/ฟอนต์ = `user/web/theme.css` — ทั้งหมดรอดทุกอัปเดต ส่วน animation ลึกๆ ให้ AI ช่วยแก้ได้ — ระบบออกแบบเผื่อไว้แล้ว: โซนหน้าตาแก้พังยังไง ส่วนเงินไม่พังตาม ดูวิธีใน [VIBECODE.md](VIBECODE.md) แก้เสร็จรัน `make verify` เขียว = โอเค แดง = undo

**Q: อัปเดตเวอร์ชันใหม่แล้วของที่ปรับแต่งไว้หายไหม?**
ไม่หาย — ทุกอย่างที่คุณแก้ (`user/` กับ `.env`) ระบบอัปเดตไม่แตะเลย และข้อมูล tip อยู่ใน
docker volume แยกอีกชั้น วิธีอัปเดตดูหัวข้อ "อัปเดตระบบเป็นเวอร์ชันใหม่" ด้านบน
